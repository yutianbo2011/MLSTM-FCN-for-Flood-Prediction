...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
                cutoff=None, normalize_timeseries=False, learning_rate=1e-3, monitor='val_loss', optimization_mode='auto', compile_model=True):
    ...
    early_stopping = EarlyStopping(monitor=monitor, min_delta = 1e-3, patience=25, verbose=1, mode='auto',baseline=None, restore_best_weights=False)
    ...
    data=np.concatenate((np.array(training_acc_list).reshape((length, 1)), np.array(val_acc_list).reshape((length, 1)),
                         np.array(training_loss_list).reshape((length, 1)), np.array(val_loss_list).reshape((length, 1))),
                        axis=1)
    df=pandas.DataFrame(data, columns=['training_accuracy', 'validation_accuracy', 'training_loss', 'validation_loss'])
